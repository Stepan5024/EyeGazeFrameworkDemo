...
from server.utils.utils import prep_input_numpy, getArch
from server.models.gaze_result_container import GazeResultContainer
...
class Pipeline:
    ...
    def step(self, frame: np.ndarray) -> GazeResultContainer:
        ...
        # Save data
        if bboxes != []:
            _bboxes = np.stack(bboxes)
        else:
            _bboxes = np.empty((0,1))
        if landmarks != []:
            _landmarks = np.stack(landmarks)
        else:
            _landmarks = np.empty((0,1))
        if scores != []:
            _scores = np.stack(scores)
        else:
            _scores = np.empty((0,1))

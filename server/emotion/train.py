...
def save_model_onnx(model, input_size, file_name="./resources/models/emotion/emotion_model_video.onnx"):
    model.eval()  # Set the model to evaluation mode

    # Create a dummy input tensor with the correct size and type
    dummy_input = torch.randn(input_size, dtype=torch.float32, device='cpu')  # Match the dtype with your model's expected input
    ...
    try:
        torch.onnx.export(model,
                          ...
                          file_name,
                          export_params=True,
                          opset_version=12,  # Using a newer opset version
                          do_constant_folding=True,
                          input_names=['input'],
                          output_names=['output'],
                          dynamic_axes={'input' : {0 : 'batch_size'},  # Optional: handle variable batch sizes
                                        'output' : {0 : 'batch_size'}})
...
def main():
    ...
    for epoch in range(0, cfg.NUM_OF_EPOCHS):
        ...
        if device == "cuda":
            model = model.to("cpu")
        torch.save(model.state_dict(), './resources/models/emotion/emotion_model_video.pth')
        ...
        plt.savefig('./resources/models/emotion/emotion_model_video.jpg')

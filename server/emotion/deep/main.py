...
def Train(epochs,train_loader,val_loader,criterion,optmizer,device):
    ...
    torch.save(net.state_dict(),'deep_emotion-{}-{}-{}.pth'.format(epochs,batchsize,lr))
    print("===================================Training Finished===================================")
    dummy_input = torch.randn(1, 1, 48, 48, device=device)

    # Export the model to an ONNX file
    net.eval()
    output_onnx = 'deep_emotion.onnx'
    torch.onnx.export(net, dummy_input, output_onnx, export_params=True, opset_version=11,
                      do_constant_folding=True, input_names=['input'], output_names=['output'],
                      dynamic_axes={'input': {0: 'batch_size'}, 'output': {0: 'batch_size'}})
    print(f"Model has been converted to ONNX format and saved as {output_onnx}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Configuration of setup and training process")
    parser.add_argument('-s', '--setup', type=bool, help='setup the dataset for the first time')
    parser.add_argument('-d', '--data', type=str,required= True,
                               help='data folder that contains data files that downloaded from kaggle (train.csv and test.csv)', default="F:\\EyeGazeDataset\\emotion\\chell\\")
    ...
    parser.add_argument('-lr', '--learning_rate', type= float, help= 'value of learning rate', default=0.01)
    parser.add_argument('-bs', '--batch_size', type= int, help= 'training/validation batch size', default=64)
